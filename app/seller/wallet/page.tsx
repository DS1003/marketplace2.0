...
import { Wallet, ArrowUp2, ArrowDown2, Building, Mobile, Import, DollarCircle, History, TrendUp, ShieldTick, ArrowRight2, Flash, Activity, Clock } from "reicon-react"
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Flash className="w-3.5 h-3.5 text-teal-600 fill-teal-600" />
        ...
        <div className="flex gap-2">
          ...
          <WithdrawalButton disabled={balance <= 0} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Plafond Mensuel", value: "5.0M", icon: TrendUp, color: "text-emerald-500 bg-emerald-50" },
            { label: "Retraits Actifs", value: "1", icon: Flash, color: "text-teal-500 bg-teal-50" },
          ].map((s, i) => (
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest text-teal-600 hover:text-teal-700 cursor-pointer flex items-center gap-1 transition-colors">Exporter CSV <Import className="w-3 h-3" /></span>
        </div>
        ...
        <Card className="rounded-[1.5rem] border-zinc-200/50 shadow-sm bg-white overflow-hidden">
               ...
               { id: 4, title: 'Paiement Service', ref: 'SVC-1102-L', amount: -5000, type: 'out', date: '30 Mars 2026' },
                ...
                <div key={tx.id} className="p-5 flex items-center justify-between hover:bg-teal-50/10 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      tx.type === 'in' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100/50' : 'bg-rose-50 text-rose-600 border border-rose-100/50'
                       ...
                       {tx.type === 'in' ? <DollarCircle className="w-4 h-4" /> : <Mobile className="w-4 h-4" />}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <ArrowRight2 className="w-3.5 h-3.5 text-slate-200 group-hover:text-teal-600 transition-colors" />
                  </div>
                </div>
             ))}
          </div>
          <div className="p-4 bg-slate-50/50 text-center border-t border-zinc-100 hover:bg-teal-50/40 transition-colors cursor-pointer group">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-teal-600 flex items-center justify-center gap-2 transition-colors">
              Synchroniser l'historique complet <ArrowRight2 className="w-3 h-3" />
            </span>
          </div>
        </Card>
      </div>
