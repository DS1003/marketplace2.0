...
export default async function SellerLayout({
  ...
  const isPending = shop?.status === "PENDING" && session.user.role !== "SUPER_ADMIN"
      ...
      <aside className="w-60 fixed inset-y-0 bg-[#0F172A] text-slate-400 overflow-hidden shadow-2xl border-r border-white/5 z-50 flex flex-col transition-all duration-300">
        ...
        <nav className="flex-1 px-3 mt-4 space-y-0.5 overflow-y-auto custom-scrollbar">
          {sellerNavItems.map((item) => {
            // Restriction for pending sellers
            if (isPending && (item.href !== "/seller" && item.href !== "/seller/messages" && item.href !== "/seller/settings")) {
              return null;
            }
        ...
        <div className="p-3">
           <div className="bg-white/[0.03] rounded-2xl p-4 space-y-3 border border-white/5">
              ...
              <LogoutButton className="w-full justify-start h-8 rounded-lg gap-2 text-slate-500 hover:text-rose-400 hover:bg-rose-400/5 text-[9px] font-black uppercase tracking-widest px-2 shadow-none border-none" />
           </div>
        </div>
      </aside>
      ...
      <main className="flex-1 lg:pl-60 flex flex-col min-w-0">
        <header className="h-14 bg-white/40 backdrop-blur-md border-b border-zinc-200/50 flex items-center justify-between px-8 sticky top-0 z-30">
           <div className="flex items-center gap-6 flex-1">
              <h2 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] hidden lg:block">System Status: <span className="text-teal-500">{isPending ? "Verification" : "Active"}</span></h2>
